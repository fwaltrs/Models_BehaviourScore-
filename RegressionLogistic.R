...
saveRDS(vc_lasso, file = "H:/Meu Drive/9º SEMESTRE/TG/Regressao_logistica/base_sem_cat/reg_logis_3_seed29.rds")
...
graf_pos <- ggplot(data=coef_pos[2:16,],aes(x=reorder(Variavel,
  ...
  geom_bar(stat="identity",col="white",fill="blue4")+
  coord_flip()+
...
ic_inferior <- risco-1.96*sqrt(var_risco/length(conjunto_teste)) #0.2023094
ic_superior <- risco+1.96*sqrt(var_risco/length(conjunto_teste)) #0.3188498
...
g1 <- ggplot(gerar_df(teste$faixas_score), aes(x = Categoria)) +
  ...
  geom_line(aes(y = Taxa_Inadimplencia * 100, group = 1),
  ...
  scale_y_continuous(limits = c(0,100), 
                     sec.axis = sec_axis(~./100, name = "Taxa de Inadimplência (%)")) +
  theme_minimal()+
  scale_fill_manual(values = cores) +  
  theme(
    plot.title = element_text(hjust = 0.5,size=14),
